...
public class CreateAssetTool(ILogger<CreateAssetTool> logger, UnityWebSocketService webSocketService)
{
    ...
    [McpServerTool]
    [Description("Create any type of Unity asset (Material, Texture2D, AnimationClip, ScriptableObject, etc.) using reflection. Allows setting properties via JSON. The asset will be saved in the Assets folder with the appropriate extension.")]
    public async Task UnityCreateAssetAsync(
